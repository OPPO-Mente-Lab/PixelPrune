...
def deduplicate_consecutive_exact(tensor: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
    """精确匹配去重（exact = max, threshold=0）。"""
    return deduplicate_consecutive(tensor, method='exact')


def deduplicate_consecutive_mse(
    tensor: torch.Tensor,
    threshold: float = 0.01
) -> Tuple[torch.Tensor, torch.Tensor]:
    """基于 RMSE 的连续去重。threshold 单位：[0,1] 空间每像素 RMS 误差。"""
    return deduplicate_consecutive(tensor, method='rmse', threshold=threshold)


def deduplicate_consecutive_max_error(
    tensor: torch.Tensor,
    threshold: float = 0.01
) -> Tuple[torch.Tensor, torch.Tensor]:
    """基于最大像素误差（L∞）的连续去重。threshold 单位：[0,1] 空间单像素最大误差。"""
    return deduplicate_consecutive(tensor, method='max', threshold=threshold)
...
def create_sequence_mask(
    ...
    """
    创建序列 mask，True 表示有效位置。

    Args:
        lengths: [batch_size]
        max_len: 最大长度（默认 lengths.max()）
